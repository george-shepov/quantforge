...
class CourseScenarioRunner:
    ...
    def _verdict(labs: list[dict[str, Any]]) -> str:
        ...
        if "assumption_failure" in classifications:
            return "reject"
        if "data_quality_warning" in classifications or "explainable_deviation" in classifications:
            return "revise"
    ...
    def _run_experiment_lab(
        ...
        deltas = [
            compare_expectation(
                "candidate_count",
                {"kind": "minimum", "value": 1, "reason": "At least one candidate must be evaluated."},
                len(candidates),
            ),
            compare_expectation(
                "fold_count",
                {"kind": "minimum", "value": 1, "reason": "Sequential evaluation requires a non-empty test fold."},
                len(candidates[0]["folds"]) if candidates else 0,
            ),
        ]
        if monte_carlo_runs:
            deltas.append(
                compare_expected_actual(
                    "monte_carlo.runs",
                    monte_carlo_runs,
                    candidates[0]["monte_carlo"].get("runs") if candidates else None,
                )
            )
        return self._generic_scenario(lab_id, raw, request, actual, deltas, seed, git_sha)
    ...
    def _generic_scenario(
        ...
        scenario = ExecutableScenario(
            ...
            delta_classification=classify_deltas(deltas) if deltas else "data_quality_warning",
