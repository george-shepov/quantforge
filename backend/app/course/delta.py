...
def compare_direction(
    ...
    if direction == "increase":
        ...
        satisfied = _numeric(baseline) and _numeric(actual) and actual < baseline
    elif direction in {"may_change", "may_decrease", "must_be_measured"}:
        satisfied = _numeric(baseline) and _numeric(actual)
    else:
        return ScenarioMeasurement(
            metric=metric,
            expected=direction,
            actual=actual,
            classification=DeltaClassification.DATA_QUALITY_WARNING,
            explanation=f"Unsupported direction rule: {direction}.",
        )
    classification = (
        DeltaClassification.MATCHED
        if satisfied
