...
class OrderBookSnapshot:
    ...
    def from_payload(cls, payload: dict) -> "OrderBookSnapshot":
        ...
        try:
            ...
            raw_symbol = str(payload["symbol"]).strip()
            venue_symbol = str(payload.get("venue_symbol", raw_symbol)).strip()
            symbol = canonical_symbol(payload.get("canonical_symbol", raw_symbol))
...
def simulate_order(
    ...
    *,
    ...
    if not math.isfinite(quantity) or quantity <= 0 or (side != Side.BUY and side != Side.SELL):
        return ExecutionResult(quantity, 0.0, quantity, None, OrderStatus.REJECTED, reason="invalid_parameter")
    if not math.isfinite(fee_bps) or fee_bps < 0:
        return ExecutionResult(quantity, 0.0, quantity, None, OrderStatus.REJECTED, reason="invalid_parameter")
    if limit_price is not None and (not math.isfinite(limit_price) or limit_price <= 0):
        return ExecutionResult(quantity, 0.0, quantity, None, OrderStatus.REJECTED, reason="invalid_parameter")
    if snapshot.crossed:
        return ExecutionResult(quantity, 0.0, quantity, None, OrderStatus.REJECTED, reason="crossed_book")
    if now_ms is not None and snapshot.timestamp_ms > now_ms:
        return ExecutionResult(quantity, 0.0, quantity, None, OrderStatus.REJECTED, reason="future_dated_snapshot")
    if snapshot.is_stale(now_ms=now_ms, max_age_ms=max_age_ms):
        return ExecutionResult(quantity, 0.0, quantity, None, OrderStatus.REJECTED, reason="stale_snapshot")
    ...
    if not levels:
        ...
        return ExecutionResult(quantity, 0.0, quantity, None, status, reason="insufficient_depth")
    remaining = quantity
    fills: list[Fill] = []
    ...
    if filled_quantity <= 0:
        status = OrderStatus.OPEN if limit_price is not None else OrderStatus.REJECTED
        reason = "limit_not_crossing" if limit_price is not None else "insufficient_depth"
    elif remaining > 1e-12:
        status = OrderStatus.PARTIALLY_FILLED
        reason = "limit_not_crossing_remaining" if stopped_by_limit else "insufficient_depth"
    else:
        status = OrderStatus.FILLED
        reason = None
    return ExecutionResult(quantity, filled_quantity, max(0.0, remaining), average_price, status, fills, reason)
